...
**Auth UI pages implemented** (`api-endpoints-implementation-plans/registration-post-endpoint-implementation-plan.md`):
...
- `src/app/[locale]/auth/callback/{page.tsx,CallbackClient.tsx}` — server-side `exchangeCodeForSession(code)` (PKCE for OAuth + email-confirmation); client-side `flushPendingConsent()` then redirects. `?next` is whitelisted to internal paths only (no `//`) to block open-redirect.
