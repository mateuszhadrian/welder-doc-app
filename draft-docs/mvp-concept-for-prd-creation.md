...
3. **Manipulacja i parametryzacja**
  - Możliwość precyzyjnej regulacji wszystkich wymiarów za pomocą suwaków z dokładnością do 0,1 mm; użytkownik może w dowolnej chwili przełączyć jednostki (np. na cale lub jednostki niestandardowe według potrzeb). DO EDYCJI - jednostki nieistotne (tylko proporcje)
...
4. **Tryb sekwencji spawania (uproszczony)**
  ...
  - Możliwość nadawania numeracji ściegów ręcznie(np. 1, 2, 3… lub A, B, C… itp.).
5. **Eksport dokumentacji**
  - **Priorytet główny**: Eksport do plików graficznych (JPG, PNG) – rysunek złącza + numeracja
