...
import { BusinessError, mapPostgrestError } from '@/lib/supabase/errors';
...
export async function migrateGuestAutosave(
  ...
  const { error } = await supabase.from('documents').insert({
    owner_id: userId,
    name: documentName,
    data: autosave.scene as unknown as Database['public']['Tables']['documents']['Insert']['data']
  });
  ...
  if (error) {
    ...
    // Surface the underlying Postgres error for telemetry / dev debugging.
    // Sentry/log shipper will pick this up post-MVP; for now it lands in the
    // browser console where Playwright tests can capture it.
    console.error('[migrateGuestAutosave] documents.insert failed', {
      code: error.code,
